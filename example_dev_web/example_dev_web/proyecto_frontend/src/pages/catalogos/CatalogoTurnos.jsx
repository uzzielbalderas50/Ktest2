...
import FormTurno from '../../components/catalogos/forms/FormTurno'
import ConfirmationModal from '../../components/ui/ConfirmationModal'
import SubNav from '../../components/ui/SubNav'
import { useToast } from '../../components/ui/ToastContext'
import { useTurnos, useBuscarTurnos, useDeleteTurno } from '../../hooks/useTurnos'
import { useExcel } from '../../hooks/useExcel'

const subNavLinks = [
  { to: '/app/catalogos/areas',    label: 'Areas' },
  { to: '/app/catalogos/turnos',   label: 'Turnos' },   // ← agrega aquí tu ruta
]
...
const CatalogoTurnos = () => {
  ...
  const queryAll    = useTurnos()
  const querySearch = useBuscarTurnos(debouncedSearch)
  ...
  const handleDescargarExcel = () => {
    ...
    descargar(todos, [
      { header: 'ID',          accessor: r => r.turno_id },
      { header: 'Nombre',      accessor: r => r.nombre },
      { header: 'Hora Inicio', accessor: r => r.hora_inicio },
      { header: 'Hora Fin',    accessor: r => r.hora_fin },
    ], 'Catalogo_Turnos')
  }

  const handleOpenForm = (turno = null) => { setSelectedTurno(turno); setIsSlideOverOpen(true) }
  const confirmDelete  = () => {
    if (!turnoToDelete) return
    deleteMutation.mutate(turnoToDelete, {
      onSuccess: () => { showToast('Turno eliminado correctamente', 'success'); setTurnoToDelete(null) },
      onError:   () => { showToast('Error al eliminar', 'error');               setTurnoToDelete(null) }
    })
  }
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="relative w-80">
            ...
            <input type="text" placeholder="Buscar turno..." value={search}
          ...
          <div className="flex items-center space-x-2">
            ...
            <button onClick={() => handleOpenForm(null)}
              className="bg-white text-[#003588] border border-[#003588] hover:bg-[#003588] hover:text-white transition-colors rounded-none flex items-center text-sm font-medium px-4 py-2 focus:outline-none">
              <Plus className="h-5 w-5 mr-2" />Nuevo Turno
            </button>
          </div>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left font-semibold text-gray-700 uppercase tracking-wider">Inicio</th>
                  <th className="px-6 py-3 text-left font-semibold text-gray-700 uppercase tracking-wider">Fin</th>
                  <th className="px-6 py-3 text-center font-semibold text-gray-700 uppercase tracking-wider w-32">Acciones</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {currentQuery.isLoading ? (
                  <tr><td colSpan="5" className="px-6 py-4 text-center text-gray-500">Cargando turnos...</td></tr>
                ) : data.length === 0 ? (
                  <tr><td colSpan="5" className="px-6 py-4 text-center text-gray-500">No se encontraron turnos</td></tr>
                ) : currentItems.map((item) => (
                  <tr key={item.turno_id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-gray-900">{item.turno_id}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-900">{item.nombre}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-900">{item.hora_inicio}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-900">{item.hora_fin}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      ...
                      <button onClick={() => setTurnoToDelete(item.turno_id)} className="text-gray-400 hover:text-[#003588] hover:bg-gray-100 p-1 mx-1 focus:outline-none" title="Eliminar"><Trash className="h-4 w-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {/* Modals */}
      <FormTurno isOpen={isSlideOverOpen} onClose={() => setIsSlideOverOpen(false)} turnoToEdit={selectedTurno} />
      <ConfirmationModal
